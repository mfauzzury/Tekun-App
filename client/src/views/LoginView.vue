  ...
  <div class="flex min-h-screen flex-col items-center justify-center bg-[#f6f9fc] px-4">
    <div class="w-full max-w-[400px]">
      ...
      <div class="rounded-lg border border-[#e3e8ee] bg-white px-10 pb-10 pt-8 shadow-[0_2px_4px_rgba(0,0,0,0.05),0_1px_2px_rgba(0,0,0,0.06)]">
        <h1 class="mb-1 text-center text-xl font-semibold tracking-tight text-[#1a1f36]">{{ t('auth.signInTitle') }}</h1>
        <p class="mb-8 text-center text-[13px] text-[#697386]">{{ site.siteTitle || 'Admin' }} {{ t('auth.dashboardSuffix') }}</p>

        <form class="space-y-5" @submit.prevent="submit">
          <div class="space-y-1.5">
            <label class="text-[13px] font-medium text-[#1a1f36]">{{ t('auth.email') }}</label>
            <input
              v-model="email"
              type="email"
          ...
          <div class="space-y-1.5">
            <label class="text-[13px] font-medium text-[#1a1f36]">{{ t('auth.password') }}</label>
            <div class="relative">
              <input
                v-model="password"
          ...
          >
            {{ auth.loading ? t('auth.signingIn') : t('auth.continue') }}
